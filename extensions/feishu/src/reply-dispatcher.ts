...
export function createFeishuReplyDispatcher(params: CreateFeishuReplyDispatcherParams) {
  ...
  // Timing tracking for end-to-end response time
  let replyStartTime = 0;
  const showTiming = account.config?.showTiming ?? true;
  ...
  const closeStreaming = async () => {
    ...
    if (streaming?.isActive()) {
      let text = streamText;
      // Append timing info for streaming mode
      if (showTiming && replyStartTime > 0) {
        const elapsed = ((Date.now() - replyStartTime) / 1000).toFixed(1);
        text = text + `\n\n⏱️ 此次回复共花费 ${elapsed} 秒`;
      }
  ...
  const { dispatcher, replyOptions, markDispatchIdle } =
    core.channel.reply.createReplyDispatcherWithTyping({
      ...
      deliver: async (payload: ReplyPayload, info) => {
        ...
        // Append timing info to final message (only for non-streaming mode)
        // Note: Streaming mode timing is handled in closeStreaming()
        if (
          info?.kind === "final" &&
          showTiming &&
          replyStartTime > 0 &&
          !(streamingEnabled && streaming?.isActive())
        ) {
          const elapsed = ((Date.now() - replyStartTime) / 1000).toFixed(1);
          text = text + `\n\n⏱️ 此次回复共花费 ${elapsed} 秒`;
        }
        ...
        if (shouldDeliverText) {
          ...
          if (streaming?.isActive()) {
            ...
            if (info?.kind === "final") {
              // Remove any existing timing info before merging to avoid duplication
              const textWithoutTiming = text.replace(/\n\n⏱️ 此次回复共花费 [0-9.]+ 秒$/g, "");
              streamText = mergeStreamingText(streamText, textWithoutTiming);
